...
def simulate(cars):
    ...
    random_cars = [car.travelTime for car in cars if (car.type == CarType.random and car.startTime <= 8 and car.travelTime > 0)]
...
cars.extend([Car(env, i + car_nro,  graph.getFirstNode(), graph, random.uniform(0, SIMULATION_TIME-3), CarType.random) for i in range(NUM_CARS)])
car_nro += NUM_CARS

cars.extend([Car(env, i + car_nro,  graph.getFirstNode(), graph,  random.uniform(8, 13), CarType.intelligent, graph.getLastNode()) for i in range(300)])
car_nro += 200
